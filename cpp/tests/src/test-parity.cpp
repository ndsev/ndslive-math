...
int main()
{
    ...
    for (const auto& r : data["nds_to_wgs84"]) {
        ...
        if (x <= -(1LL << 31) || x >= (1LL << 31) - 1 ||
            y <= -(1LL << 30) || y >= (1LL << 30) - 1)
            continue;
        Wgs84<double> w = Wgs84<double>::fromNdsCoordinates(static_cast<int32_t>(x),
    ...
    for (const auto& r : data["packed_tile_from_index"]) {
        ...
        CHECK_EQ(static_cast<int64_t>(t.mortonNumber()), r["computed_morton_number"].get<int64_t>());
    ...
    for (const auto& r : data["tiles_for_bbox"]) {
        auto tiles = ndsmath::getTileIdsForBoundingBox(
            r["sw_x"].get<int32_t>(), r["sw_y"].get<int32_t>(),
            r["ne_x"].get<int32_t>(), r["ne_y"].get<int32_t>(), r["level"].get<int>());
    ...
    for (const auto& r : data["nds_bbox_ops"]) {
        auto a = r["a"]; auto b = r["b"];
        NdsBoundingBox ba(a[0].get<int32_t>(), a[1].get<int32_t>(), a[2].get<int32_t>(), a[3].get<int32_t>());
