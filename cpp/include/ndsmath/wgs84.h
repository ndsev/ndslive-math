...
template <typename T> class Wgs84 : public glm::vec<2, T, glm::highp>
{
public:
...
public:
    ...
    Wgs84(T longitude, T latitude) : vec2_t(longitude, latitude) { normalize(); }

public:
    operator vec2_t const &() const { return *this; }

    inline vec2_t const &vec() const { return *this; }

    inline T const &longitude() const { return x; }

    inline T const &latitude() const { return y; }

    inline T const &dx() const { return x; }

    inline T const &dy() const { return y; }
    ...
    T distanceTo(const Wgs84 &other) const
    {
        ...
        const double a = glm::sin(dLat * 0.5) * glm::sin(dLat * 0.5) +
                         glm::cos(glm::radians(y)) * glm::cos(glm::radians(other.latitude())) *
                             glm::sin(dLon * 0.5) * glm::sin(dLon * 0.5);
    ...
    T bearingFrom(const Wgs84 &other) const
    {
        ...
        const T x = glm::cos(lat1f) * glm::sin(lat2f) -
                    glm::sin(lat1f) * glm::cos(lat2f) * glm::cos(lon2f - lon1f);
        return glm::atan(y, x);
    }
    ...
    //! @note Longitude distance varies by latitude (shrinks toward poles), latitude distance is
    //! constant.
    ...
    //! @return Pair of (width_meters, height_meters)
    static std::pair<T, T> ndsDistanceToMeters(int32_t ndsXDistance, int32_t ndsYDistance,
                                               T atLatitude)
    {
    ...
    glm::dvec3 toEuclidean(T refRadius = EARTH_RADIUS_IN_METERS) const
    {
        ...
        return {refRadius * std::cos(theta) * std::sin(phi), refRadius * std::sin(theta),
    ...
    Wgs84<T> &move(T distanceInMeters, T bearing)
    {
        ...
        T lat = glm::asin(glm::sin(latitudeInRad) * glm::cos(angularDistance) +
                          glm::cos(latitudeInRad) * glm::sin(angularDistance) * glm::cos(bearing));

        T dlon = glm::atan(glm::sin(bearing) * glm::sin(angularDistance) * glm::cos(latitudeInRad),
                           glm::cos(angularDistance) - glm::sin(latitudeInRad) * glm::sin(lat));

        T lon = longitudeInRad + dlon + glm::pi<T>();

        while (lon > glm::two_pi<T>())
            lon -= (glm::two_pi<T>());
    ...
    bool operator!=(const typename Wgs84<T>::vec2_t &other) const { return !(*this == other); }
    ...
    bool isNull() const { return Wgs84<T>(x, y) == Wgs84<T>(); }
...
template <typename T> using DeltaInMeters = std::pair<T, T>;
