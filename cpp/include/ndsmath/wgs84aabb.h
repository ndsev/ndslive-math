...
template <typename T> class Wgs84AABB
{
public:
    ...
    explicit Wgs84AABB(PackedTileId const &tileId)
        : Wgs84AABB(Wgs84<T>::fromMortonCode(tileId.southWestCorner()),
                    glm::abs((Wgs84<T>::fromMortonCode(tileId.southWestCorner()) -
                              Wgs84<T>::fromMortonCode(tileId.northEastCorner()))
                                 .vec()))
    {
    }

    /// Construct the AABB from a center position, a tile count limit, and a tile level
    static Wgs84AABB<T> fromCenterAndTileLimit(Wgs84<T> const &center, uint32_t softLimit,
                                               uint16_t level)
    {
        ...
        auto targetSizeVec = typename Wgs84<T>::vec2_t{targetSize / targetAspectRatio,
    ...
    inline Wgs84<T> const &sw() const { return sw_; }

    /// Obtain the North-East corner of this AABB.
    inline Wgs84<T> ne() const { return sw_ + size_; }

    /// Obtain the North-West corner of this AABB.
    inline Wgs84<T> nw() const { return sw_ + vec2_t{.0, size_.y}; }

    /// Obtain the South-East corner of this AABB.
    inline Wgs84<T> se() const { return sw_ + vec2_t{size_.x, .0}; }

    /// Obtain all four vertices, one for each corner of the AABB.
    inline std::vector<Wgs84<T>> vertices() const { return {sw(), se(), ne(), nw()}; }
    ...
    inline Wgs84<T> center() const { return sw_ + size_ * .5; }
    ...
    inline double avgMercatorStretch()
    {
        ...
        auto radToMercatorLat = [](T wgs84Lat)
    ...
    {
        return point.longitude() >= sw_.longitude() &&
               point.longitude() <= sw_.longitude() + size_.x &&
               point.latitude() >= sw_.latitude() && point.latitude() <= sw_.latitude() + size_.y;
    }
