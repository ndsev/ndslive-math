...
/**
 * Free-function utilities for working with collections of
 * {@link PackedTileId}s.
 *
 * <p>
 * Ports the module-level functions {@code get_tile_ids_for_bounding_box} and
 * {@code bounding_box_from_tile_ids} from
 * {@code python/src/ndslive/math/tileid.py}.
 * </p>
 */
public final class TileIds {
	 ...
	 *
	 * <p>
	 * Uses floor division ({@link Math#floorDiv}) to compute tile indices so that
	 * negative coordinates are handled correctly (toward negative infinity),
	 * matching Python's {@code //}.
	 * </p>
	 *
	 * @param swX
	 *            south-west corner X (longitude) in NDS coordinates
	 * @param swY
	 *            south-west corner Y (latitude) in NDS coordinates
	 * @param neX
	 *            north-east corner X (longitude) in NDS coordinates
	 * @param neY
	 *            north-east corner Y (latitude) in NDS coordinates
	 * @param level
	 *            tile level (0-15)
	 * @return the list of PackedTileIds that intersect the bounding box
	 */
	public static List<PackedTileId> getTileIdsForBoundingBox(long swX, long swY, long neX, long neY, int level) {
	 ...
	 *
	 * <p>
	 * The north-east corner of a tile is exclusive, so the returned max values have
	 * 1 subtracted to be the last point inside the coverage, matching
	 * {@code bounding_box_from_tile_ids} in the reference.
	 * </p>
	 *
	 * @param tiles
	 *            a non-empty list of PackedTileIds
	 * @return a {@code long[]} of {@code {sw_x, sw_y, ne_x, ne_y}} in NDS coords
	 * @throws IllegalArgumentException
	 *             if {@code tiles} is empty
	 */
