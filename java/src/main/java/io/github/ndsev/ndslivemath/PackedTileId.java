 ...
 *
 * <p>Per the NDS.Live standard, tile IDs are signed 32-bit integers. For levels
 * 0-14, values are positive. For level 15, values are negative
 * ({@code -2147483648} to {@code -1}) because the level bit (bit 31) is the sign
 * bit in signed int32 representation.</p>
 *
 * <p>Internally values are stored as unsigned 32-bit integers (in a
 * {@code long} masked to {@code 0xFFFFFFFF}) to enable clean bit operations.
 * Conversion between signed and unsigned happens only at the API boundary:
 * the constructor accepts both representations and {@link #value()} returns the
 * signed int32 per the standard.</p>
 *
 * <p>Faithful port of {@code python/src/ndslive/math/tileid.py}.</p>
 */
public final class PackedTileId {
     ...
     *
     * <p>Accepts both signed and unsigned 32-bit integer representations. Level
     * 15 tiles have negative signed values ({@code -2147483648} to {@code -1});
     * levels 0-14 have positive values.</p>
     *
     * @param value tile ID as signed int32 (negative for level 15) or unsigned
     ...
     *
     * @param mortonNumber the tile's morton number ({@code 0} to {@code 2^(2*level+1) - 1})
     * @param level tile level (0-15)
     * @return the PackedTileId with the given morton number at the given level
     * @throws IllegalArgumentException if level or mortonNumber are out of range
     */
    ...
    /**
     * @return the south-west corner of the tile in NDS coordinates as {@code {x, y}}
     */
    ...
    /**
     * Returns the north-east corner of the tile in NDS coordinates. This corner
     * is <em>exclusive</em> (the first point outside the tile).
     *
    ...
    private void validate() {
        ...
        if (this.value < minPackedTileId) {
            throw new IllegalArgumentException(
                    "Invalid PackedTileId(" + value() + "): value must be >= " + minPackedTileId
        ...
        if (morton < 0 || morton > maxMorton) {
            throw new IllegalArgumentException(
                    "Invalid PackedTileId(" + value() + "): morton number " + morton
                            + " exceeds valid range for level " + tileLevel + " (allowed: 0-"
                            + maxMorton + ")");
        }
    }
